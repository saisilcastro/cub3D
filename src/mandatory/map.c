...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/25 12:36:21 by lde-cast          #+#    #+#             */
/*   Updated: 2024/03/25 12:36:30 by lde-cast         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <cub3d.h>

inline static t_status	get_map_size(t_map *level, char **arr)
{
	int		i;
	char	*line;
...
inline static t_status	checker(t_map *map, char **texture)
{
	...
	{	
		if (!ft_strncmp(line, "NO ", 3))
			...
			map->textures[3] = texture_breaker(line, "EA ");
		else if (!ft_strncmp(line, "SO ", 3))
			map->textures[2] = texture_breaker(line, "SO ");
		else if (!ft_strncmp(line, "WE ", 3))
			map->textures[1] = texture_breaker(line, "WE ");
...
t_status	get_map(char *map_fd, t_map *level)
{
	...
	i = -1;
	while (array[++i] && *array[i] && On)
		if (!checker(level, &array[i]))
			break ;
	if (!get_map_size(level, array + i) || !map_validate(level, array + i))
		ok = Off;
	array_pop(array);
	return (ok);
}
