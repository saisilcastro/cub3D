...
/*   Updated: 2024/03/25 23:06:31 by mister-code      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_status	map_validate(t_map *lv, char **array)
{
	...
	while (array[++p.y])
	{
		...
		while (array[p.y][++p.x])
		{
			if (special_char(array[p.y][p.x]))
					count_player += swap_char(cub_get(), array[p.y], p.y);
			if (ft_isdigit(array[p.y][p.x]))
				lv->map[p.y][p.x] = array[p.y][p.x] - 0x30;
		}
	}
	lv->map[p.y] = NULL;
